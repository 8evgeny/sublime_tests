...
//Строим таблицу кодирования
    void BuildTable(node *root)
    {
        if (root->left!=nullptr)
                          { code.push_back(0);
                          BuildTable(root->left);} // Рекурсия

        if (root->right!=nullptr)
                           { code.push_back(1);
                           BuildTable(root->right);}

        if (root->left==nullptr && root->right==nullptr) tabl[root->c]=code; //лист

        code.pop_back(); // один символ из vector убираем - фактически встаем на предыдущий элемент
    }

    void PrintTable()
    {
        cout<<endl<<"--------------------    CODDING TABL   ------------------------- "<<endl;
        int l = 1;
        for (auto i:tabl)
        {
            cout<<l<<"\t"<<i.first<<" - ";
            auto itboolvector = i.second.begin();
            for(itboolvector= i.second.begin(); itboolvector != i.second.end(); ++itboolvector )
            {
            cout<<*itboolvector;
            }
            cout<<endl;
...
// Вывод помощи о пользовании программой
    void help ()
    {
    printf("encoding name_input_file\n");
    }
...
// Перемещаем указатель в начало файла
        ...
        in_sourse.clear(); in_sourse.seekg(0);
...
// Для декодирования нам нужно будет построить дерево - для этого нужно передать все символы
// и частоту вхождения каждого символа
// Выдаем колл. символов таблицы, колл символов сжатого файла и три массива
    FILE *out;
    char *NAME = argv[1];
...
//    fwrite (argv[1], (sizeof argv[1]+1), 1, out);
    ...
    cout<<"Creating file "<<"CODDING_DATA.HUFFMAN"<<" - "<<lenth_out<<" bytes"<<endl;
