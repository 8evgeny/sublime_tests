...
void example() {
  ...
  cout << "Available : " << space.available / 1024 / 1024 / 1024 << "GB"
       << endl;
  cout << endl;
}
}  // namespace example_a
...
vector<string> getDirectoryFiles(const fs::path &dir,
  ...
  for (auto &p : fs::recursive_directory_iterator(dir)) {
    if (fs::is_regular_file(p)) {
      if (ext.empty() || find(ext.begin(), ext.end(),
                              p.path().extension().string()) != ext.end())
...
void example() {
  ...
  vector<string> res_files = {"./res/pics/file.png", "./res/pics/file.jpg",
                              "./res/pics/file.info"};

  for (auto &f : res_files) ofstream(f) << "data";

  auto print_files = [](const auto &aVector) {
    for (auto &f : aVector) cout << f << endl;
    cout << endl;
  };
...
void example() {
  ...
  fs::permissions("file_test", fs::perms::add_perms | fs::perms::owner_all |
                                   fs::perms::group_all);
  ...
  fs::permissions("file_test",
                  fs::perms::remove_perms | fs::perms::owner_write |
