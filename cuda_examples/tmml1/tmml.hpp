...
class tmml
{
  public:
    ...
    std::thread arr_th[numCudaTread];

    const Pix max_pix0;
    void work_tmml(const cv::Mat& img_work, const cv::Mat& img_temp, Pix& max_pix);
    void maxValue(Pix * dev_mp, Pix& max);
    void thr_gpu(int i, const cv::cuda::PtrStepSz<unsigned char> img_work_gpu, Pix * host_mp);
//    void gpu_work(const cv::Mat& img_work, int i);
  ...
  private:
    ...
    cv::cuda::GpuMat img_temp_gpu, img_work_gpu;
