...
void SocketAddress::set_ip(const std::string& ip)    {_ip = ntohl(inet_addr(ip.c_str()));}

void SocketAddress::set_nip(const uint32_t& nip)   {_ip = ntohl(nip);}
...
void SocketAddress::set_nport(const uint16_t& nport) {_port = ntohs(nport);}

uint32_t SocketAddress::nip() const {return htonl(_ip);}

uint16_t SocketAddress::nport() const {return htons(_port);}
