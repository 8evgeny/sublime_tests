...
void Application::exec()
{
    ...
    if(parallel)
    {
        while(!frameReady.load()) // ждём флаг получения первого изображения
        {
...
#ifdef USE_TRAC
    mtrac_ptr = make_shared<mtrac::Multitrac>(config_path, frame_process_0, &device, ok);
    if(!ok)
    {
        cout << "Application::NOT OK create mtrac_ptr: quit!" << endl;
        quit();
        return;
    } // END if(!ok)
#endif // USE_TRAC
#ifdef USE_TKDNN
    tkdnn_ptr = make_shared<TKDNNContainer>(config_path, frame_process_0, ok);
    if(!ok)
    {
        cout << "Application::NOT OK create tkdnn_ptr: quit!" << endl;
        quit();
        return;
    } // END if(!ok)
#endif // USE_TKDNN

    cout << "Application:: continue exec..." << endl;
...
#ifdef USE_LOGGER
    time_point_workflow0 = system_clock::now();;
#endif // USE_LOGGER
    cout << "dbg:: Start main cycle!" << endl;
    

    // основной цикл выполнения пиложения
    while(_execute.load(memory_order_acquire))
    {
        next_frame_sync();      // получение кадра, обработка флагов и информации для LOGGER при parallel == 0
        if(frameReady.load())   // если кадр получен
        {
            next_frame_async(); // получение кадра, обработка флагов и информации для LOGGER при parallel == 1
            //            cout << "dbg:: frame[" << device->getId() << "] is ready" << endl;
            //            cout << "New cycle!" << endl;
            bool find_some = 0;
#ifdef USE_TKDNN
            find_some = tkdnn_ptr->isFind();
#endif // USE_TKDNN
            if(video_on && (rec_on || find_some)) // запись фрейма (при влючении или нахождении объекта нейросетью)
            {
            ...
            //            cout << "dbg:: recorder[" << device->getId() << "] is ready" << endl;
#ifdef USE_TRAC
            mtrac_ptr->prepare_v_trac(frame_receive);
#endif // USE_TRAC
#ifdef USE_TKDNN
            tkdnn_ptr->prepareFrameForTKDNN(frame_receive);
#endif // USE_TKDNN
            ...
            time_point_workflow1 = system_clock::now();
            LoggerArtem::inst().logTimedBasedFPS("Full FPS = ",
                                                 duration<double>(time_point_workflow1 - time_point_workflow0).count());
            time_point_workflow0 = system_clock::now();
#endif // END USE_LOGGER

#ifdef USE_TRAC
            mtrac_ptr->update(frame_receive, frame_show);
#endif //USE_TRAC
#ifdef USE_TKDNN
            bool flag_zahvat = tkdnn_ptr->update(aimRectShats, frame_show);
#endif // END USE_TKDNN
            //cout << "dbg:: application end update" << endl;
#ifdef USE_CORSAIR_400_RAW
            // "Заправляем" найденные рамки в структуру
            track_pos_data2send.v_track.resize(0);
#ifdef USE_TRAC // !USE_TKDNN
            if(flag_zahvat)
            {
                for(list<shared_ptr<mtrac::TracContainer>>::iterator it = mtrac_ptr->l_trac_ptr.begin(); it != mtrac_ptr->l_trac_ptr.end(); it++)
                {
                    tr_pos.x = it->operator*().aimRectShats.x;
                    tr_pos.y = it->operator*().aimRectShats.y;
                    tr_pos.w = it->operator*().aimRectShats.width;
                    tr_pos.h = it->operator*().aimRectShats.height;
                    track_pos_data2send.v_track.emplace_back(tr_pos);
                } // END for(list<trac_data>::iterator it = tkdnn_ptr->trac_nnet_ptr->ltrac_data_ptr->begin(); it != tkdnn_ptr->trac_nnet_ptr->ltrac_data_ptr->end(); it++)
                track_pos_data2send.v_track.emplace_back(tr_pos);
            } // END if(flag_zahvat)
#endif // END #ifdef USE_TRAC
            sender_ptr->send(track_pos_data2send);
#ifdef USE_TKDNN
            //            vector<trac_data>& vtd = {0};

            for(list<trac_data>::iterator it = tkdnn_ptr->trac_nnet_ptr->ltrac_data_ptr->begin(); it != tkdnn_ptr->trac_nnet_ptr->ltrac_data_ptr->end(); it++)
            {
                tr_pos.x = it->x;
                tr_pos.y = it->y;
                tr_pos.w = it->w;
...
#endif // USE_TRAC
#endif // USE_LOGGER
#if defined GUI_OK && defined USE_TKDNN

#endif // GUI_OK

#if defined(USE_CORSAIR_400_RAW)/* && !defined (USE_TKDNN)*/
            if(ethHandler(rectm))
            {
                handle_treatment();
            } // -- END if(EthHandler(frame_show, rect))
#endif // USE_ETH_CONTROLLER && !USE_TKDNN
#ifdef GUI_OK
            //            cout << "dbg:: mouseHandler[" << device->getId() << "] is start" << endl;
            if(mouseHandler(frame_show, rectm))
            {
                cout << "dbg:: mouseHandler[" << device->getId() << "] is ok" << endl;
                handle_treatment();
            } // END if(mouseHandler(frame_show, rect))
            //            cout << "dbg:: mouseHandler[" << device->getId() << "] is ready" << endl;
...
void Application::next_frame_sync()
{
    if(!parallel)
    {
#ifdef USE_LOGGER
...
bool Application::get_ini_params(const string &config)
{
    ...
    num_trac_max = reader.GetInteger("main_settings", "num_trac_max", -1);
    if(num_trac_max == -1)
    {
        cout << "Not found roi_mode in Application::getMainSettings!\n";
        return false;
    } // END if(num_trac_max == -1)
...
void Application::handle_treatment()
{
    bool ok = false;
#ifdef USE_TRAC
    if(mtrac_ptr->add_trac(rectm, frame_receive, frame_show, &device))
    {
        cout << "Application::handle_treatment:: add_trac() is OK!" << endl;
    } // END if(add_trac)
    else
    {
        cout << "Application::handle_treatment:: error add_trac!" << endl;
    } // END if(!add_trac)
#endif // USE_TRAC
    cout << "Application::End add trac" << endl;
    //    mtracShats->init_trac(rectm, frame_show);
    return;
...
bool Application::ethHandler(cv::Rect2f & rect_obj)
{
    ...
    if(cmd_list2keep.init_f)
    {
        ...
        //        mtrac_ptr->add_trac(rect_obj, frame_receive, frame_show, device, 0);
...
bool Application::mouseHandler(Mat& img, Rect2f& object_rect)
{
    if(handle_flag)
    {
        ...
        //#endif // END #ifndef TKDNN
    } // END if(handle_flag)
    if(rec_on)
    {
        circle(img, Point(30, 60), 10, color::white, -1);
    } // END if(rec_on)
    ...
    if(handle_flag)
    {
        ...
        //        cout << "dbg:: wh_2_zahvat = " << wh_2_zahvat << endl;
        if(!isTracShatsInitedFlag)
        {
...
void Application::keyHandler()
{
    ...
    {
        cout << "Application::KeyHandler::Exit on keypress" << endl;
        break;
    } // END case '`'
    default:
#ifdef USE_TKDNN
        Rect2f tkdnn2trac_rct = {0,0,0,0};
        if(tkdnn_ptr->keyHandler(key, tkdnn2trac_rct))
        {
            bool ok = 0;
            cout << "\n Application::keyHandler::START add track from TKDNN in " << tkdnn2trac_rct << "!" << endl;
            tkdnn2trac_rct.x *= frame_show.cols; // переводим по размеру frame_show, т.к. функция mtrac_ptr->add_trac рассчитана
            tkdnn2trac_rct.y *= frame_show.rows;
            tkdnn2trac_rct.width *= frame_show.cols;
            tkdnn2trac_rct.height *= frame_show.rows;
            cout << "dbg::Application::KeyHandler::  tkdnn2trac_rct = " << tkdnn2trac_rct << endl;
            cout << "dbg::Application::KeyHandler:: frame_show.size = " << frame_show.size() << endl;
            cout << "dbg::Application::KeyHandler:: frame_receive.size = " << frame_receive.size() << endl;
            tkdnn2trac_rct.x -= bord2init;
            tkdnn2trac_rct.y -= bord2init;
            tkdnn2trac_rct.width += bord2init2;
            tkdnn2trac_rct.height += bord2init2;
            cout << "dbg:: Application::KeyHandler::  tkdnn2trac_rct = " << tkdnn2trac_rct << endl;
            ok = mtrac_ptr->add_trac(tkdnn2trac_rct, frame_receive, frame_show, &device, tkdnn_ptr->getIDmain());
            if(ok)
            {
                cout << " Application::keyHandler::OK add track from TKDNN!" << endl;
            } // END if(ok)
            else
            {
                cout << " Application::keyHandler::OK add track from TKDNN!" << endl;
            } // END else
        } // END if(tkdnn_ptr->keyHandler(key, tkdnn2trac_rct))
#endif // USE_TKDNN
        break;
    } // -- END switch(key)
} // -- END keyHandler
