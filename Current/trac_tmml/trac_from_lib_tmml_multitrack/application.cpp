...
void Application::exec()
{
    ...
    //    mtracShats = make_shared<mtrac::TracContainer>(config_path, frame_process_0, &device, ok);
    cout << "Application::Multitrac constructor = " << ok << endl;
#else // END !USE_MULTITRAC
    tracShats = make_shared<TracShats>(config_path, frame_process_0, ok);
#endif // END !USE_MULTITRAC
    ...
    while(_execute.load(memory_order_acquire))
    {
        ...
        if(frameReady.load())   // если кадр получен
        {
...
#ifdef USE_MULTITRAC
            bool find_some = mtrac_ptr->check_find_in_ltrac();
            //            bool find_some = check_find_in_ltrac();
            //            cout << "Application::exec::check_find_in_ltrac = " << find_some << endl;
#else // !USE_MULTITRAC
            bool find_some = tracShats->find_;
#endif // !USE_MULTITRAC
...
#ifdef USE_MULTITRAC
            //          cout << "Application::start prepare_v_trac" << endl;
            mtrac_ptr->prepare_v_trac(frame_receive);
            //          cout << "Application::end prepare_v_trac" << endl;
            //          mtracShats->prepareFrameForTracShats(frame_receive);
#else // !USE_MULTITRAC
            prepareFrameForTracShats();
#endif // USE_MULTITRAC
            //          cout << "dbg:: prepareFrameForTracShats[" << device->getId() << "] is ready" << endl;
            ...
            //            mtracShats->workflowShats(frame_receive, frame_show);
            //            cout << "dbg:: workflowShats[" << device->getId() << "] is ready" << endl;
#else // if TKDNN
            work_TKDNN();
#endif // END #ifdef TKDNN
        ...
        else
        {
            ...
            //            mtracShats->tracShats->trac_str.new_frame = 0;
            ...
            //                mtracShats->tracShats->trac_str.new_frame = 1;
...
{
    bool ok = false;
    mtrac_ptr->add_trac(rectm, frame_receive, frame_show, &device, ok);
...
void Application::keyHandler()
{
    ...
    case 27: // деинциализация трекера по клавише ESC
    {
        ...
        break;
        if(mtracShats->tracShats->isInited())
        {
            cout << "dbg::Application::KeyHandler:: last element is inited" << endl;

            mtracShats->tracShats->deinit();
            //            mtracShats->workflowShats(frame_receive, frame_show);
            cout << "last element inited aftetr deinit =  " << mtracShats->tracShats->isInited() << endl;
        } // END if(tracShats->isInited())
        else
        {
            cout << "dbg::Application::KeyHandler:: last element isn't inited" << endl;
            if(mtracShats->tracShats->isInited())
            {
                mtracShats->tracShats->deinit();
                //                mtracShats->workflowShats(frame_receive, frame_show);
                cout << "last element inited aftetr deinit =  " << mtracShats->tracShats->isInited() << endl;
            } // END if(mtracShats->tracShats->isInited())
        } // END else
        cout << "dbg::Application::KeyHandler:: END treatment 'esc'" << endl;
        break;
    } // END  case 27:
#endif // END #ifndef TKDNN
