...
void TracContainer::workflowShats(cv::Mat & frame_receive, cv::Mat & frame_show, int pos, int pos_main)
{
    ...
    if(isTracShatsFirstInitedFlag)
    {
        ...
        {
#ifdef USE_CUDA
            frame_show(Rect(pt_text - Point(7, 7), pt_text + Point(7, 7))) = color::black;
            frame_show(Rect(pt_text - Point(3, 3), pt_text + Point(3, 3))) = color::white;
...
#ifdef USE_CL
            frame_show(Rect(pt_text - Point(7, 7), pt_text + Point(7, 7))) = color::white;
            frame_show(Rect(pt_text - Point(3, 3), pt_text + Point(3, 3))) = color::black;
        ...
        if(flag_zahvat)
        {
            ...
            if(demonstration_mode > 1)
            {
                ...
                if(demonstration_mode > 3)
                {
                    ...
                    string txt = "TracShats " + to_string(pos) + ": Object size=[" + to_string(objSize.x) + ", " + to_string(objSize.y) + "]";
                    if(pos == pos_main)
                    {
                        putText(frame_show, txt, pt_text + Point(20, 10), 2, 0.8, clr_pos_main, thik_pos);
                        putText(frame_show, to_string(pos), Point(rectShats.tl()) + Point(2,12), 1, 1, clr_pos_main, thik_pos);
                    } // END if(pos == pos_main)
                    else
                    {
                        putText(frame_show, txt, pt_text + Point(20, 10), 2, 0.8, clr_pos, thik_pos);
...
{
    cout << "Begin Multitrac::get_ini_params[" << section << "], path: " << config_path << "\n";
    bool ok = true;
...
bool TracContainer::init_trac(Rect2f &rectm, cv::Mat & frame_show)
{
    ...
    if(!tracShats->isInited())
    {
        if(roi_mode)
        {
            ...
            if(initFlag)
            {
                cout << "Application:: OK init in " << rectm << ";\n roi = " << roi << endl;
                isRoiSetted = true;
            } // END if(initFlag)
            else
            {
        ...
        if(initFlag)
        {
            //            cout << "Application::handle treatment:: set isTracShatsFirstInitedFlag" << endl;
            if(!isTracShatsFirstInitedFlag) {isTracShatsFirstInitedFlag = true;}
            isTracShatsInitedFlag = true;
        } // -- END if(initFlag)
    } // -- END if(!tracShats->isInited())
