#ifndef TRACTKDNN_HPP
#define TRACTKDNN_HPP
...
{
 public:
    TracTKDNN(const std::string& config_path, cv::Mat& frame_process_0, bool& ok);
    ~TracTKDNN();

    std::shared_ptr<nnet> trac = nullptr;
    ...
    std::shared_ptr<std::list<trac_data>> ltrac_data_ptr = nullptr;
//    std::list<trac_data> * ltrac_ptr;
