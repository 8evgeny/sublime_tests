...
#include "multitrac/multitrac.hpp"
#endif // USE_TRAC

#ifdef USE_TKDNN
#include "tractkdnn/tractkdnn.hpp"
#include "multitrac/tkdnn_container.hpp"
#endif // USE_TKDNN
...
private:
    ...
    float handle_flag = 1; // разрешение ручного захвата мышью в случае работы по ethernet(иначе всегда 1)
    ...
    cv::Rect2f searchRect;                          // поожение трекера на новом кадре
    cv::Rect2f rectm;                               // рамка трекера для инициализации нового трека (rect mouse)
...
#ifdef USE_TRAC
    std::shared_ptr<mtrac::Multitrac> mtrac_ptr = nullptr;
#endif // USE_TRAC
    ...
    time_point_workflow0, time_point_workflow1;
...
#ifdef USE_TKDNN
    std::shared_ptr<TKDNNContainer> tkdnn_ptr = nullptr;
    cv::Mat frame_process_tkdnn;    // кадр для обработки трекером
    bool isRoiSetted = false;           // признак установки ROI
    int bord2init = 7 ;             // размер увеличения рамки по сравнению с найденным нейросетью
    int bord2init2 = 2 * bord2init ;

    inline void prepareFrameForTKDNN();
    bool work_TKDNN();
    void execTKDNN();

//    atomic_bool f_create_ptr = false;
//    atomic_bool f_new_frame2tkdnn = false;
//    atomic_bool f_exec_tkdnn = false;
//    atomic_bool f_first_cycle = true;
//    mutex mtx_tkdnn;
    ...
    void eth_handle_treatment();

#endif // USE_CORSAIR_400_RAW

    void next_frame_sync();     // получение кадра, обработка флагов и информации для LOGGER при parallel == 0
    void next_frame_async();    // получение кадра, обработка флагов и информации для LOGGER при parallel == 1
    bool check_find_in_ltrac();
