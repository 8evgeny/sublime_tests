...
extern const char * label_txt_path;
extern int class_num_obj;
char * labels = new char[class_num_obj];
...
static char *readLine(FILE *fp, char *buffer, int *len)
{
    ...
    *len = buff_len;
    ...
    return buffer;
} // END static char *readLine(FILE *fp, char *buffer, int *len)
...
static int readLines(const char *fileName, char *lines[], int max_line)
{
    ...
    fclose(file);
    return i;
} // END static int readLines(const char *fileName, char *lines[], int max_line)

static int loadLabelName(const char *locationFilename, char *label[])
{
//    printf("load lable %s\n", locationFilename);
    readLines(locationFilename, label, class_num_obj);
    return 0;
} // END static int loadLabelName(const char *locationFilename, char *label[])
...
} // END static float CalculateOverlap
...
static int quick_sort_indice_inverse(std::vector<float> &input, int left, int right, std::vector<int> &indices)
{
    ...
    return low;
} // END static int quick_sort_indice_inverse
...
} // END static void compute_dfl(float* tensor, int dfl_len, float* box)
                      ...
                      float threshold)
{
    ...
    for (int i = 0; i < grid_h; i++)
    {
        for (int j = 0; j < grid_w; j++)
        {
            ...
            // Используйте сумму баллов для быстрой фильтрации
            if (score_sum_tensor != nullptr){
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    return 0;
} // END int post_process

int init_post_process()
{
    int ret = 0;
    ret = loadLabelName(label_txt_path, &labels);
    if (ret < 0)
    {
    ...
    return 0;
} // END int init_post_process()

char * class_to_name(int cls_id)
{
    if (cls_id >= class_num_obj)
    {
        return "null";
    }
    ...
    return "null";
} // END char * class_to_name(int cls_id)

void deinit_post_process()
{
    delete [] labels;

//    for (int i = 0; i < class_num_obj; i++)
//    {
//        if (&labels[i] != nullptr)
//        {
//            free(&labels[i]);
