...
class nnet
{
 public:
   nnet(){}
   nnet(const std::string& config_path, bool& ok); // -- Конструктор
   ~nnet();
 ...
 private:
   ...
   string data_way, data_way_track, cfg, cfg_track, weights, weights_track, weights_rt_batch1, weights_rt_batch1_track, classes_file;
   float y_scan_max, work_h_w, min_deflect, max_deflect, cfg_w_2, cfg_h_2, nms_thres, box_conf_thres;
   int shift_x, shift_y, fr_w1, fr_h1, fr_h2, x_scan_max2, y_scan_max2, y_scan_max0, y_scan_max1, y_max0, class_num, obj_class_num;
