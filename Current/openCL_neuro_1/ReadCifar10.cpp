            ...
            for (int i = 0; i < number_of_images; ++i)
            {
                ...
                for (int ch = 0; ch < 3; ++ch)
                {
                    ...
                    for (int r = 0; r < n_rows; ++r)
                    {
                        ...
                        }//END for (int c = 0; c < n_cols; ++c)
                    }//END for (int r = 0; r < n_rows; ++r)
                    channels.push_back(tp);

                }//END for (int ch = 0; ch < 3; ++ch)
                merge(channels, fin_img);
                vec.push_back(fin_img);
                label.ATD(0, i) = (float)tplabel;
            }//END for (int i = 0; i < number_of_images; ++i)
        }//END if (file.is_open())
}//END void read_batch
...
Mat concatenateMat(vector<Mat> &vec)
{
    ...
    for (int i = 0; i<vec.size(); i++)
    {
        ...
        cvtColor(vec[i], gray, cv::COLOR_BGR2RGB);
    ...
    return res;
}//END Mat concatenateMat(vector<Mat> &vec)
