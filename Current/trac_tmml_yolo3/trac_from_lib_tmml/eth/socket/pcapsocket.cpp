...
bool PcapSocket::opn()
{
    ...
    std::stringstream ss;
    ss << "ip && udp && (dst net " << _address.str_ip() << " || dst net 255.255.255.255) && dst port " << _address.port();
...
bool PcapSocket::set_non_blocking(const bool & non_blocking)
{
...
void PcapSocket::fill_headers()
{
    ...
    packet->eth_hdr.from_addr[0] = _address.mac()[5];
    packet->eth_hdr.from_addr[1] = _address.mac()[4];
    packet->eth_hdr.from_addr[2] = _address.mac()[3];
    packet->eth_hdr.from_addr[3] = _address.mac()[2];
    packet->eth_hdr.from_addr[4] = _address.mac()[1];
    packet->eth_hdr.from_addr[5] = _address.mac()[0];
    // IP Header
    ...
    packet->ip_hdr.crc = 0;
    packet->ip_hdr.saddr[0] = _address.pip()[3];
    packet->ip_hdr.saddr[1] = _address.pip()[2];
    packet->ip_hdr.saddr[2] = _address.pip()[1];
    packet->ip_hdr.saddr[3] = _address.pip()[0];
    // UDP Header
    packet->udp_hdr.sport = _address.nport();
