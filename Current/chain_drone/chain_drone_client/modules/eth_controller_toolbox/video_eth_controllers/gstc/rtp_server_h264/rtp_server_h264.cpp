...
std::atomic<bool> RtpServerH264::new_frame = {false};
Fenix2Meta RtpServerH264::meta_data_srv_glob = Fenix2Meta();

static void setAppSrcSettings(GstElement *appsrc,
                              const server::h264::VideoSettings &settings)
{
...
static void setRtpPaySettings(GstElement *rtppay,
                              const server::h264::PaySettings &settings)
{
...
static void setUdpSettings(GstElement *udpsink,
                           const server::h264::UdpSettings &settings)
{
...
static void setEncoderSettings(GstElement *encoder,
                               const server::h264::EncoderSettings &settings)
{
...
GstPadProbeReturn rtp::RtpServerH264::set_x_to_rtp_header1(GstPad *pad,
...
namespace rtp
{
void waitData(GstElement *appsrc, guint unused, rtp::RtpServerH264 *server)
{
...
RtpServerH264::RtpServerH264(server::h264::SettingsPtr &settings_ext) //: settings(settings)
{
    settings = settings_ext;
}

bool RtpServerH264::setup()
{
    gst_init(NULL, NULL);
...
bool RtpServerH264::start()
{
    ...
    std::thread streametTh(&RtpServerH264::workflow, this);
    streametTh.detach();
...
void RtpServerH264::newFrame(uint8_t *buffer, size_t size)
{
...
void RtpServerH264::setMeta(Fenix2Meta & meta_ext)
{
    meta_data_srv_glob = meta_ext;
    return;
} // -- END setMeta

void RtpServerH264::workflow()
{
