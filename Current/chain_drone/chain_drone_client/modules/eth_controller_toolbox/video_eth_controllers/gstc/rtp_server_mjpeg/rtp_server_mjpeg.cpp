...
std::atomic<bool> RtpServerMJPEG::new_frame = {false};
Fenix2Meta RtpServerMJPEG::meta_data_srv_glob = Fenix2Meta();

static void setAppSrcSettings(GstElement *appsrc,
                              const server::mjpeg::VideoSettings &settings)
{
...
static void setRtpPaySettings(GstElement *rtppay,
                              const server::mjpeg::PaySettings &settings)
{
...
static void setUdpSettings(GstElement *udpsink,
                           const server::mjpeg::UdpSettings &settings)
{
...
static void setEncoderSettings(GstElement *encoder,
                               const server::mjpeg::EncoderSettings &settings)
{
...
GstPadProbeReturn rtp::RtpServerMJPEG::set_x_to_rtp_header1(GstPad *pad,
...
namespace rtp
{
void waitData(GstElement *appsrc, guint unused, rtp::RtpServerMJPEG *server)
{
...
RtpServerMJPEG::RtpServerMJPEG(server::mjpeg::SettingsPtr &settings_ext) //: settings(settings)
{
    settings = settings_ext;
}

bool RtpServerMJPEG::setup()
{
    gst_init(NULL, NULL);
...
bool RtpServerMJPEG::start()
{
    ...
    std::thread streametTh(&RtpServerMJPEG::workflow, this);
    streametTh.detach();
...
void RtpServerMJPEG::newFrame(uint8_t *buffer, size_t size)
{
...
void RtpServerMJPEG::setMeta(Fenix2Meta & meta_ext)
{
    meta_data_srv_glob = meta_ext;
    return;
} // -- END setMeta

void RtpServerMJPEG::workflow()
{
