...
Fenix2Meta RtpClientH265::fenix2meta_data_glob = Fenix2Meta();

GstPadProbeReturn RtpClientH265::get_x_from_rtp_header (GstPad          *pad,
...
static void setUdpSrcSettings(GstElement *rtspsrc,
                              const client::h265::UdpSettings &settings)
{
...
//                 NULL);
    g_object_set(rtspsrc, "location", "rtsp://Alex:Shatskiy@192.168.1.64:554/Streaming/channels/1", NULL);
    g_object_set(rtspsrc, "latency", "0", NULL);
//    g_object_set(G_OBJECT(rtspsrc), "caps",
//                 gst_caps_new_simple("application/x-rtp",
...
static void setWatchdogSettings(GstElement *watchdog, const client::h265::WatchdogSettings &settings)
{
    g_object_set(watchdog, "timeout", settings.timeout_ms, NULL);
}

static void setAppSinkSettings(GstElement *appsink,
                               const client::h265::VideoSettings &settings,
                               const client::h265::AppsinkSettings &appsink_settings)
{
...
static void setRtpDepaySettings(GstElement *rtpdepay,
                                const client::h265::DepaySettings &settings)
{
...
static void setParserSettings(GstElement *parser,
                              const client::h265::ParserSettings &settings)
{
...
static void setDecoderSettings(GstElement *decoder,
                               const client::h265::DecoderSettings &settings)
{
...
namespace rtp
{
void newData(GstElement *appsink, RtpClientH265 *client)
{
...
RtpClientH265::RtpClientH265(client::h265::SettingsPtr &settings) : settings(settings)
{
...
bool RtpClientH265::setup()
{
    gst_init(NULL, NULL);
    g_printerr("START setup()\n");
    sync.frameReady.store(false);

    components.rtspsrc = gst_element_factory_make("rtspsrc", "udp");
    components.rtpdepay = gst_element_factory_make("rtph265depay", "depay");

    components.parser = gst_element_factory_make("h265parse", "parser");
    components.queue = gst_element_factory_make("queue", "queue");
    components.decoder = gst_element_factory_make("avdec_h265", "decoder");
    ...
    rtph265depay_sink_pad = gst_element_get_static_pad(components.rtpdepay, "sink");
    gst_pad_add_probe (rtph265depay_sink_pad, GST_PAD_PROBE_TYPE_BUFFER,
                       (GstPadProbeCallback) get_x_from_rtp_header, NULL, NULL);
    gst_object_unref (rtph265depay_sink_pad);


    //    GST_ERROR()
    /// DBG::
    message_ok(!components.rtspsrc, "udpsrc");
    ...
    if (!pipeline || !components.rtspsrc || !components.rtpdepay ||
    ...
    setUdpSrcSettings(components.rtspsrc, settings->udp);
    ...
    gst_bin_add_many(GST_BIN(pipeline), components.rtspsrc,
                     components.rtpdepay, components.parser,
                     components.decoder, components.converter,
                     components.watchdog,
    ...
    gst_element_link_many(components.rtspsrc,
                          components.rtpdepay, components.parser,
                          components.decoder, components.converter,
                          components.watchdog,
                          components.appsink, NULL);
...
bool RtpClientH265::start()
{
    ...
    sync.execute = true;
    std::thread streametTh(&RtpClientH265::workflow, this);
    streametTh.detach();

    return true;
}

bool RtpClientH265::stop()
{
    // clean up
...
bool RtpClientH265::is_execute()
{
    return sync.execute.load();
}

bool RtpClientH265::frame(cv::Mat &frame, Fenix2Meta &fenix2meta_data)
{
