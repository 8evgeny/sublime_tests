#include "async_tcp_server_simplex_cmd_factory.hpp"
...
std::shared_ptr<AsyncTcpServerSimplexCmd> create(const std::string &path2ini, const std::string &section)
{
    std::cout << "START create eth_controller::cmd::async::server_simplex::AsyncTcpServerSimplexCmd" << std::endl;
    Settings settings;
    if (!readIniSettings(path2ini, section, settings))
    {
        std::cout << "AsyncTcpServerSimplexCmd factory:: Not ok readIniSettings(" << path2ini << ", " << section << ", settings)!" << std::endl;
    } // END if (!readIniSettings(path2ini, section, settings))
    std::cout << "END create eth_controller::cmd::async::server_simplex::AsyncTcpServerSimplexCmd" << std::endl;
    return std::make_shared<AsyncTcpServerSimplexCmd>(settings);
} // END create
