...
{
TcpServerSetupInfo::TcpServerSetupInfo(Settings _settings) : settings(_settings)
{
...
} // -- END TcpServerSetupInfo

TcpServerSetupInfo::~TcpServerSetupInfo()
{
    close(sockfd);
    close(connfd);
} // -- END ~TcpServerSetupInfo

bool TcpServerSetupInfo::setup(Fenix2SetupInfo & setup_info)
{
    ...
    servaddr.sin_addr.s_addr = inet_addr(settings.dst_ip_v4.c_str()); // 127.0.0.1
    servaddr.sin_port = htons(settings.dst_port);
    ...
    else
    {
        ...
        ///отправляем команду
        int buffer_size_bytes = sizeof(setup_info);
        uint8_t *cmd_buffer = new uint8_t[buffer_size_bytes];
        int bytes = 0;

        memcpy(cmd_buffer, reinterpret_cast<uint8_t const *>(&setup_info), buffer_size_bytes);

        while (bytes != buffer_size_bytes)
        {
            bytes = send(connfd, cmd_buffer, buffer_size_bytes, 0);
            std::cout << "dbg::end send! bytes = " << bytes << std::endl;
            if(bytes != buffer_size_bytes)
            {
                std::cout << "TcpServerSetupInfo:: error send cmd!" << std::endl;
                std::this_thread::sleep_for(std::chrono::milliseconds(500));
            }
        } // END while (bytes != buffer_size_bytes)
    } // END ! if (connfd < 0)
    sync.success_setup.store(true);
    return true;
} // -- END setup

bool TcpServerSetupInfo::exec()
{
    if(!sync.success_setup.load())
    {
        std::cout << "call TcpServerSetupInfo::exec() but setup not success!" << std::endl;
        return false;
    }
    ...
    std::cout << settings.node_alias << " -- TcpServerSetupInfo::disconnection done" << std::endl;
    sync.success_quit.store(true);
    std::cout << "END " << settings.node_alias << " TcpServerSetupInfo::exec()" << std::endl;
    return true;
} // -- END exec

bool TcpServerSetupInfo::quit()
{
    ...
    //     // std::cout << "Try close TcpServerSetupInfo" << std::endl;
    // }
    // close(sockfd);
    // close(connfd);
...
bool TcpServerSetupInfo::close_socket(bool force)
{
...
void TcpServerSetupInfo::register_cmd_handler(ICmdHandler *handler)
{
    cmd_handlers.push_back(handler);
} // -- END TcpServerSetupInfo

bool TcpServerSetupInfo::is_connect_established()
{
    return sync.success_setup.load();
} // -- END is_connect_established

bool TcpServerSetupInfo::is_success_quit()
{
    return sync.success_quit.load();
} // -- END is_success_quit

void TcpServerSetupInfo::register_notify_handler(IEthNotifyHandler *handler)
{
    this->notify_handlers.push_back(handler);
} // -- END register_notify_handler

void TcpServerSetupInfo::apply_notify_all_handlers(eth_notify_utils::EthNotifyType notify_type)
{
