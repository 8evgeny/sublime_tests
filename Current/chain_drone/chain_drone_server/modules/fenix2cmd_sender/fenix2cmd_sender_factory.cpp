#include "fenix2cmd_sender_factory.hpp"
#include "ExtendINIReader.hpp"

using namespace fenix2cmd_sender;
...
bool readIniSettings(const std::string &path2ini, const std::string section, Settings &settings_dst)
{
    ...
    ini_report &= ext_ini::GetInteger(reader, section, "fenix2_cmd_data_cmd_net_id", settings_dst.cmd_net_id);

    std::cout << "END " << section << "::readIniSettings" << std::endl;
    return ini_report;
} // -- END readIniSettings

std::shared_ptr<Fenix2CmdSender> fenix2cmd_sender::create(const std::string &path2ini, const std::string &section)
{
    Settings settings;
    ...
    std::shared_ptr<Fenix2CmdSender> cam_ptr = std::make_shared<Fenix2CmdSender>(settings);
    return cam_ptr;
} // -- END create
