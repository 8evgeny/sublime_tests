...
std::atomic<bool> RtpServerH265::new_frame = {false};
Fenix2Meta RtpServerH265::meta_data_srv_glob = Fenix2Meta();

static void setAppSrcSettings(GstElement *appsrc,
                              const server::h265::VideoSettings &settings)
{
    ...
    g_object_set(appsrc, "emit-signals", 1, NULL);
...
static void setRtpPaySettings(GstElement *rtppay,
                              const server::h265::PaySettings &settings)
{
...
static void setUdpSettings(GstElement *udpsink,
                           const server::h265::UdpSettings &settings)
{
...
static void setEncoderSettings(GstElement *encoder,
                               const server::h265::EncoderSettings &settings)
{
    g_object_set(encoder, "bitrate", settings.bitrate, NULL);
    g_object_set(encoder, "insert-sps-pps", 1, NULL);
...
setConvSettings(GstElement *converter,
                const server::h265::ConvSettings &settings)
{
    g_object_set(converter, "")
}

GstPadProbeReturn rtp::RtpServerH265::set_x_to_rtp_header1(GstPad *pad,
...
namespace rtp
{
void waitData(GstElement *appsrc, guint unused, rtp::RtpServerH265 *server)
{
...
RtpServerH265::RtpServerH265(server::h265::SettingsPtr &settings_ext) //: settings(settings)
{
...
bool RtpServerH265::setup()
{
    ...
    components.converter = gst_element_factory_make("nvvidconv", "converter");
    components.encoder = gst_element_factory_make("nvv4l2h265enc", "enc");
    components.rtppay = gst_element_factory_make("rtph265pay", "rtphpay");

    rtph265pay_pad = gst_element_get_static_pad(components.rtppay, "src");
    gst_pad_add_probe (rtph265pay_pad, GST_PAD_PROBE_TYPE_BUFFER,
                       (GstPadProbeCallback) set_x_to_rtp_header1, NULL, NULL);
    gst_object_unref (rtph265pay_pad);

    components.udpsink = gst_element_factory_make("udpsink", "udp");

    ///DBG::
    message_ok(!components.appsrc, "appsrc");
    message_ok(!components.converter, "converter");
    message_ok(!components.encoder, "encoder");
    message_ok(!components.rtppay, "rtppay");
    message_ok(!components.udpsink, "udpsink");
...
bool RtpServerH265::start()
{
    ...
    std::thread streametTh(&RtpServerH265::workflow, this);
    streametTh.detach();
...
void RtpServerH265::newFrame(uint8_t *buffer, size_t size)
{
...
void RtpServerH265::setMeta(Fenix2Meta & meta_ext)
{
    meta_data_srv_glob = meta_ext;
    return;
} // -- END setMeta

void RtpServerH265::workflow()
{
