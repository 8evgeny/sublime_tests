...
Fenix2Meta RtpClientRAW::fenix2meta_data_glob = Fenix2Meta();

GstPadProbeReturn RtpClientRAW::get_x_from_rtp_header (GstPad          *pad,
...
static void setUdpSrcSettings(GstElement *udpsrc,
                              const client::raw::UdpSettings &settings)
{
...
static void setWatchdogSettings(GstElement *watchdog, const client::raw::WatchdogSettings &settings)
{
    g_object_set(watchdog, "timeout", settings.timeout_ms, NULL);
}

static void setAppSinkSettings(GstElement *appsink,
                               const client::raw::VideoSettings &settings,
                               const client::raw::AppsinkSettings &appsink_settings)
{
...
static void setRtpDepaySettings(GstElement *rtpdepay,
                                const client::raw::DepaySettings &settings)
{
...
static void setParserSettings(GstElement *parser,
                              const client::raw::ParserSettings &settings)
{
...
static void setDecoderSettings(GstElement *decoder,
                               const client::raw::DecoderSettings &settings)
{
...
namespace rtp
{
    void newData(GstElement *appsink, RtpClientRAW *client)
    {
...
RtpClientRAW::RtpClientRAW(client::raw::SettingsPtr &settings) : settings(settings)
{
...
bool RtpClientRAW::start()
{
    ...
    sync.execute = true;
    std::thread streametTh(&RtpClientRAW::workflow, this);
    streametTh.detach();

    return true;
}

bool RtpClientRAW::stop()
{
    // clean up
...
bool RtpClientRAW::is_execute()
{
    return sync.execute.load();
}

bool RtpClientRAW::frame(cv::Mat &frame, Fenix2Meta &fenix2meta_data)
{
