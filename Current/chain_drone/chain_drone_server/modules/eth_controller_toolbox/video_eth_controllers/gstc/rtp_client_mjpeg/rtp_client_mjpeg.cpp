...
Fenix2Meta RtpClientMJPEG::fenix2meta_data_glob = Fenix2Meta();

GstPadProbeReturn RtpClientMJPEG::get_x_from_rtp_header (GstPad          *pad,
...
static void setUdpSrcSettings(GstElement *udpsrc,
                              const client::mjpeg::UdpSettings &settings)
{
...
static void setWatchdogSettings(GstElement *watchdog, const client::mjpeg::WatchdogSettings &settings)
{
    g_object_set(watchdog, "timeout", settings.timeout_ms, NULL);
}

static void setAppSinkSettings(GstElement *appsink,
                               const client::mjpeg::VideoSettings &settings,
                               const client::mjpeg::AppsinkSettings &appsink_settings)
{
...
static void setRtpDepaySettings(GstElement *rtpdepay,
                                const client::mjpeg::DepaySettings &settings)
{
...
static void setParserSettings(GstElement *parser,
                              const client::mjpeg::ParserSettings &settings)
{
...
static void setDecoderSettings(GstElement *decoder,
                               const client::mjpeg::DecoderSettings &settings)
{
...
namespace rtp
{
    void newData(GstElement *appsink, RtpClientMJPEG *client)
    {
...
RtpClientMJPEG::RtpClientMJPEG(client::mjpeg::SettingsPtr &settings) : settings(settings)
{
...
bool RtpClientMJPEG::setup()
{
    ...
    if(settings->udp.encodingName == "h264" || settings->udp.encodingName == "H264")
    {
        fn_parser = "h264parse";
        fn_decoder = "avdec_h264";
    }
    else if (settings->udp.encodingName == "jpeg" || settings->udp.encodingName == "JPEG")
    {
        fn_parser = "jpegparse";
        fn_decoder = "jpegdec";
    }

//    GstElementFactory * parserfactory = gst_element_factory_find("h264parse");
//    components.parser = gst_element_factory_create(parserfactory, "parser");
    components.parser = gst_element_factory_make(fn_parser.c_str(), "parser");
    components.decoder = gst_element_factory_make(fn_decoder.c_str(), "decoder");
...
bool RtpClientMJPEG::start()
{
    ...
    sync.execute = true;
    std::thread streametTh(&RtpClientMJPEG::workflow, this);
    streametTh.detach();

    return true;
}

bool RtpClientMJPEG::stop()
{
    // clean up
...
bool RtpClientMJPEG::is_execute()
{
    return sync.execute.load();
}

bool RtpClientMJPEG::frame(cv::Mat &frame, Fenix2Meta &fenix2meta_data)
{
