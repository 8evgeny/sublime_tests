#include "async_tcp_client_simplex_cmd_factory.hpp"
...
std::shared_ptr<AsyncTcpClientSimplexCmd> create(const std::string &path2ini, const std::string &section)
{
    std::cout << "START create eth_controller::cmd::async::client_simplex::AsyncTcpClientSimplexCmd" << std::endl;
    Settings settings;
    if (!readIniSettings(path2ini, section, settings))
    {
        std::cout << "AsyncTcpClientSimplexCmd factory:: Not ok readIniSettings(" << path2ini << ", " << section << ", settings)!" << std::endl;
    } // END if (!readIniSettings(path2ini, section, settings))
    std::cout << "END create eth_controller::cmd::async::client_simplex::AsyncTcpClientSimplexCmd" << std::endl;
    return std::make_shared<AsyncTcpClientSimplexCmd>(settings);
} // END create
