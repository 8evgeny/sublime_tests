...
bool path_processing::cleanDir(const std::string &path2dir)
{
    struct dirent *ent;
    DIR *dir = opendir(path2dir.c_str());
    if (dir != NULL)
    {
        /* remove all the files and directories within directory */
        while ((ent = readdir(dir)) != NULL)
        {
            std::string path_correct = "";
            if (path2dir.back() != '/') {path_correct = "/";}
            std::remove((path2dir + path_correct + ent->d_name).c_str());
