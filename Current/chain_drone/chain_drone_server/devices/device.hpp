...
public:
    ...
    virtual void keyHandler(uchar &key) = 0;
    virtual void workflow() = 0;
    virtual void updateAimPosition(std::pair<float, float> pos){
        device_error("update aim position");
    }
    ...
    virtual void set_speed(const int32_t & _x_speed, const int32_t & _z_speed){
        device_error("set_speed");
    }
