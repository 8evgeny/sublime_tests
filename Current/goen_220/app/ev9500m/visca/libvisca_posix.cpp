...
{
    int err;
 //   err = write(iface->port_fd, &packet->bytes[packet->length-1], 1);
  ...
  //  err = write(iface->port_fd, packet->bytes, packet->length);
    ...
    m_serialPort.write((char*)packet->bytes, packet->length);
...
_VISCA_send_packet(VISCAInterface_t *iface, VISCACamera_t *camera, VISCAPacket_t *packet)
{
    ...
    //qDebug()<<"VISCA BA VALUE:"<<QByteArray((char*)packet->bytes, packet->length).toHex(' ');
...
_VISCA_get_packet(VISCAInterface_t *iface)
{
    ...
    // Установка таймаута (например,1 секунды)
    timeout.tv_sec = 10;
    timeout.tv_usec = 0;

    // Ожидание данных на порту с таймаутом
    fd_set read_fds;
//FD_ZERO(&read_fds);
    //FD_SET(iface->port_fd, &read_fds);

    // m_serialPort.read(read_fds);

    // int select_result = select(iface->port_fd + 1, &read_fds, NULL, NULL, &timeout);

    // if (select_result == -1) {
    //     // Ошибка при вызове select
    //     return VISCA_FAILURE; // Или другой код ошибки
    // } else if (select_result == 0) {
    //     // Таймаут истек
    //     return VISCA_TIMEOUT; // Или другой код ошибки для таймаута
    // }
 ...
 //    }
    ...
    if(m_buff.size()==0)
        ...
        {
            QByteArray arr= m_serialPort.readAll();// (char*)iface->ibuf, 1);  //read(iface->port_fd, iface->ibuf, 1);

            m_buff.append(arr);
        }
        else
        {
            return VISCA_TIMEOUT;
        }
   // if(m_buff.data()[0] == VISCA_TERMINATOR) m_buff.removeFirst();
    int pos_i =0;


    for(int i= 0; i< m_buff.size(); i++)
    {
        uint8_t r_buf = m_buff.data()[i];
        iface->ibuf[i]= r_buf;
        if(r_buf==VISCA_TERMINATOR)
        {
            pos_i  =i;
            iface->bytes = i+1;
            break;
   ...
   // memcpy(iface->ibuf, m_buff.data(), iface->bytes);
    if(pos_i>0)
        m_buff.remove(0, iface->bytes);
    ...
    //         m_serialPort.read( (char*)&iface->ibuf[pos], 1);
    //     usleep(10);
    // }
   // iface->bytes=  pos+1;
...
{
  int fd;
    m_serialPort.setPortName(device_name);
...
//       /* control flags */
//       cfsetispeed(&iface->options, baudrate); //B9600);    /* 9600 Bds   */
