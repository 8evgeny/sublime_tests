...
void results_print_and_save(object_detect_result_list & od_results, image_buffer_t & img_buff)
{
    for (int i = 0; i < od_results.count; i++)
    {
        ...
        draw_rectangle(&img_buff, x1, y1, x2 - x1, y2 - y1, COLOR_BLUE, 3);
        char text[256];
        sprintf(text, "%s %.1f%%", class_to_name(det_result->cls_id), det_result->prop * 100);
        draw_text(&img_buff, text, x1, y1 - 20, COLOR_RED, 10);
    }//END for (int i = 0; i < od_results.count; i++)

    printf("\n");
    if (write_result_inference_in_file)
        write_image("out.png", &img_buff);
}//END void results_print_and_save

void results_save_to_vector(object_detect_result_list & od_results, image_buffer_t & img_buff, vector<tr>& vtr)
{
    for (int i = 0; i < od_results.count; i++)
    {
...
void release_resources(rknn_app_context_t & rknn_app_ctx, image_buffer_t & img_buff)
{
    ...
    {
        if (img_buff.virt_addr != NULL)
        {
            free(img_buff.virt_addr);
        }
...
//        waitKey(0);
    ...
    {
        ret = read_image(image_path, &img_buff);
        if (ret != 0)
        {
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < num_iterations; ++i)
    {
        ...
        ret = inference_yolov8_model(&rknn_app_ctx, &img_buff, &od_results);  if (ret != 0) {printf("init_yolov8_model fail! ret=%d\n", ret); release_resources(rknn_app_ctx, img_buff);  return -1; }
    ...
    results_print_and_save(od_results, img_buff);
    release_resources(rknn_app_ctx, img_buff);
