...
public:
    ...
    void set_ip(const std::string & ip) {_ip = ntohl(inet_addr(ip.c_str()));}
    void set_ip(const uint32_t & ip) {_ip = ip;}
    void set_nip(const uint32_t & nip) {_ip = ntohl(nip);}
    void set_ip(const uint8_t * ip);
    void set_port(const uint16_t & port) {_port = port;}
    void set_nport(const uint16_t & nport) {_port = ntohs(nport);}
    ...
    uint32_t nip() const {return htonl(_ip);}
    uint16_t port() const {return _port;}
    uint16_t nport() const {return htons(_port);}
