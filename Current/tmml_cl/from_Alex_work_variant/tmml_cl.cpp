...
tmml_cl::tmml_cl(bool& ok, float& min_max_Val0)
{
    ...
    // Kernels
    int clError = 0;
    tmml_cl_kernel = Kernel(program, KERNEL_tmml_cl_NAME1, &clError);
    if(clError != 0)
    {
        cout << "clError=" << clError << "; KERNEL_tmml_cl_NAME1=" << KERNEL_tmml_cl_NAME1 << " =============== !!!" << endl;
        ok = false;
        return;
    } // END if(clError != 0)
...
void tmml_cl::work_tmml(const Mat & img_work, const Mat & img_temp, Pix & max_pix)
{
    ...
    // Image 2D
    //queue.enqueueNDRangeKernel(tmml_cl_kernel, NullRange, NDRange(RESULT_WIDTH, RESULT_HEIGHT), NullRange);
    queue.enqueueNDRangeKernel(tmml_cl_kernel, NullRange, NDRange(RESULT_AREA), NullRange);
    queue.finish();
