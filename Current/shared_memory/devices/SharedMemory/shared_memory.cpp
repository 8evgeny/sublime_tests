...
using namespace devices::shared_memory;

SharedMemory::SharedMemory(const devices::shared_memory::UserSettings &settings) : settings(settings)
{
...
{    
    switch(settings.camera_mode)
    {
        ...
        default:
            std::cout << "ERROR: failed recognize shared_memory camera_mode, check ini-file. Use default 4K@30fps" << std::endl;
            gen_settings.video = VideoSettings(3840, 2160, 30);
        break;
    };
...
void SharedMemory::start()
{
    ...
    std::cout << "SharedMemory::start() -- Pipeline success create!\n";

    std::thread(&SharedMemory::exec, this).detach(); // поток извлечения кадров из pipeline
} // -- END SharedMemory::start()
...
void SharedMemory::exec()
{
    ...
    */
    ...
    {
        std::cout << "ERROR: SharedMemory square_crop_sz > video.width || video.height, use default settings" << std::endl;
...
} // -- END SharedMemory::exec()

void SharedMemory::quit()
{
    if(sync.quit_was_call.load())
    {
        return;
    }
    std::cout << "TRY close SharedMemory" << std::endl;
    ...
    while(!sync.exec_complete.load())
    {
        ...
        {
            std::cout << "FORCE quit SharedMemory" << std::endl;
            gst_cap.release();
            break;
        }
    }
    std::cout << "SUCCESS close SharedMemory" << std::endl;
    
    // std::cout << "SharedMemory::quit() -- gst pipeline success closed\n";
} // -- SharedMemory::quit()
...
{
    return false;
} // -- END SharedMemory::isBayerColorChannel()

void SharedMemory::keyHandler(uchar &key)
{

} // -- END SharedMemory::keyHandler(char &key)

void SharedMemory::workflow()
{

} // -- END SharedMemory::workflow()
...
int SharedMemory::getColorChannels()
{
    ...
    return 0;
#endif
} // -- END SharedMemory::getColorChannels()
