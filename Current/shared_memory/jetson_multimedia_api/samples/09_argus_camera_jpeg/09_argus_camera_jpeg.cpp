...
static int      CAPTURE_FPS   = 40;
static uint32_t SENSOR_MODE   = 0;
static Size2D<uint32_t> PREVIEW_SIZE (320, 240);
...
bool ConsumerThread::threadExecute()
{
    ...
    while (true)
    {
        ...
        {
std::cout<< "======== iEvent->getEventType() == EVENT_TYPE_ERROR ========="<<std::endl;
        ...
        {
std::cout<< "======== argusStatus != Argus::STATUS_OK ========="<<std::endl;
            printErrorStatus(argusStatus);
            errorStatus = true;
            break;
        }
...
#ifdef  INFINITE_CAPTURE
if (frameNumber == 1)
{
    std::cout<< "CAPTURE_TIME = INFINITE_CAPTURE" << std::endl;
}
#endif
#ifndef  INFINITE_CAPTURE
if (frameNumber == 1)
{
    std::cout<< "CAPTURE_TIME = "<< CAPTURE_TIME<< " sec" <<std::endl;
}
#endif

std::cout<< frameNumber <<": ======== receive new image ========="<<std::endl;
...
//#endif
    ...
    iSourceSettings->setFrameDurationRange(Range<uint64_t>(1e9/100, 1e9/5));
...
#endif
    {
        ...
        if (
#ifdef   PREVIEW_ON
                previewConsumerThread.errorStatus ||
#endif
            (DO_JPEG_ENCODE &&
