...
bool ConsumerThread::threadExecute()
{
    ...
    while (!m_gotError)
    {
        ...
        if (DO_CPU_PROCESS) {
            ...
            params = nvbuf_surf->surfaceList[0];

            uint8_t *ptr_cur;
            int i, j, a, b;

            if (!NvBufSurfaceMap (nvbuf_surf, 0, Y_INDEX, NVBUF_MAP_READ_WRITE))
            {
                ORIGINATE_ERROR("Failed to map Y plane from nvbuf_surf!");
                break;
            }
            if (!NvBufSurfaceSyncForCpu (nvbuf_surf, 0, Y_INDEX))
            {
                ORIGINATE_ERROR("Failed to sync Y plane for CPU!");
                break;
            }
            ptr_cur = (uint8_t *)nvbuf_surf->surfaceList->mappedAddr.addr +
                params.planeParams.pitch[Y_INDEX] * START_POS + START_POS;
            ...
            for (i=0; i < FONT_SIZE; i++) {
                ...
                ptr_cur = (uint8_t *)nvbuf_surf->surfaceList->mappedAddr.addr +
                    params.planeParams.pitch[Y_INDEX] * (START_POS + i) +
                    START_POS;
            }
            if (!NvBufSurfaceSyncForDevice (nvbuf_surf, 0, Y_INDEX))
            {
                ORIGINATE_ERROR("Failed to sync Y plane for Device!");
                break;
            }
            if (!NvBufSurfaceUnMap(nvbuf_surf, 0, Y_INDEX))
            {
                ORIGINATE_ERROR("Failed to unmap nvbuf_surf!");
                break;
            }
        }
...
{
    int ret = 0;
std::cout << "==== ConsumerThread::createVideoEncoder() ====" << std::endl;
                                                   ...
                                                   void *arg)
{
    ...
    ++num_capture;
std::cout<< num_capture <<": ConsumerThread::encoderCapturePlaneDqCallback" <<std::endl;
...
static bool execute()
{
    ...
    UniqueObj<OutputStreamSettings> streamSettings(iCaptureSession->createOutputStreamSettings(STREAM_TYPE_BUFFER));
    IBufferOutputStreamSettings *iStreamSettings = interface_cast<IBufferOutputStreamSettings>(streamSettings);
    if (!iStreamSettings)
...
std::cout<<"==== iBufferSettings->setEGLImage ===="<<std::endl;
...
std::cout<<"==== iCaptureSession->stopRepeat() ===="<<std::endl;
    ...
    for (uint32_t i = 0; i < NUM_BUFFERS; i++)
    {
std::cout<<"==== Destroy the EGLImages ===="<<std::endl;
