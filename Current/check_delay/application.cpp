...
void Application::exec()
{
    ...
    while(_execute.load(memory_order_acquire))
    {
        if(!parallel)
        {
            ...
            //                "Device FPS = ", duration<double>(time_point_device1 - time_point_device0).count());
#endif // END USE_LOGGER
            frameReady.store(true);
        } // END if(!parallel)
        ...
        if(frameReady.load())
        {
            ...
            delay_find(); //Основная логика приложения
            if (need_break) break;
#endif //GUI_OK
            ...
            {
                circle(frame_show, Point(0.5*frame_show_width, 0.5*frame_show_height), 0.5*frame_show_height, color::white, 1);
            } // END if(tracShats->trac_str.work_in_round)
            if(mouseHandler(frame_show, key, rectm))
            {
                cv::Scalar lev = cv::mean(frame_show);
                auto level = static_cast<int>(lev.conj()[0]);
                cout << "level: " << level << "\r\n";
            ...
            } // -- END if(mouseHandler(frame_show, key, rect))
            if(need_quit)
            {
                cout << "need quit" << endl;
                quit();
...
bool Application::get_ini_params(const string &config)
{
    ...
    cout << "Begin read settings delay_find " << endl;
    tresh_bright_min = reader.GetReal("delay_find", "tresh_bright_min", -1);
    if(tresh_bright_min == -1)
    {
        cout << "\ttresh_bright_min not declared!!" << endl;
        return 0;
    }
    else
    {
        cout << "\ttresh_bright_min = " << tresh_bright_min << ";\n";
    }
    num_flash = reader.GetInteger("delay_find", "num_flash", -1);
    if(num_flash == -1)
    {
        cout << "\tnum_flash not declared!!" << endl;
        return 0;
    }
    else
    {
        cout << "\tnum_flash = " << num_flash << ";\n";
    }
    num_level_div = reader.GetInteger("delay_find", "num_level_div", -1);
    if(num_level_div == -1)
    {
        cout << "\tnum_level_div not declared!!" << endl;
        return 0;
    }
    else
    {
        cout << "\tnum_level_div = " << num_level_div << ";\n";
    }
    flash_duration = reader.GetInteger("delay_find", "flash_duration", -1);
    if(flash_duration == -1)
    {
        cout << "\tflash_duration not declared!!" << endl;
        return 0;
    }
    else
    {
        cout << "\tflash_duration = " << flash_duration << ";\n";
    }
    min_time_between_flash = reader.GetInteger("delay_find", "min_time_between_flash", -1);
    if(min_time_between_flash == -1)
    {
        cout << "\tmin_time_between_flash not declared!!" << endl;
        return 0;
    }
    else
    {
        cout << "\tmin_time_between_flash = " << min_time_between_flash << ";\n";
    }
    fps = reader.GetInteger("delay_find", "fps", -1);
    if(fps == -1)
    {
        cout << "\tfps not declared!!" << endl;
        return 0;
    }
    else
    {
    ...
    max_time_between_flash = reader.GetInteger("delay_find", "max_time_between_flash", -1);
    if(max_time_between_flash == -1)
    {
        cout << "\tmax_time_between_flash not declared!!" << endl;
        return 0;
    }
    else
    {
        cout << "\tmax_time_between_flash = " << max_time_between_flash << ";\n";
    }

    x = reader.GetInteger("delay_find", "x", -1);
    if(x == -1)
    {
        cout << "\tx not declared!!" << endl;
        return 0;
    }
    else
    {
    ...
    y = reader.GetInteger("delay_find", "y", -1);
    if(y == -1)
    {
        cout << "\ty not declared!!" << endl;
        return 0;
    }
    else
    {
    ...
    w = reader.GetInteger("delay_find", "w", -1);
    if(w == -1)
    {
        cout << "\tw not declared!!" << endl;
        return 0;
    }
    else
    {
    ...
    h = reader.GetInteger("delay_find", "h", -1);
    if(h == -1)
    {
        cout << "\th not declared!!" << endl;
        return 0;
    }
    else
    {
    ...
    num_wait = reader.GetInteger("delay_find", "num_wait", -1);
    if(num_wait == -1)
    {
        cout << "\tnum_wait not declared!!" << endl;
        return 0;
    }
    else
    {
        cout << "\tnum_wait = " << num_wait << ";\n";
    }
...
#ifdef GUI_OK
bool Application::mouseHandler(Mat &img, uchar& key, Rect2f &object_rect)
{    
    ...
    rectangle(img, rct_work, color::white, 2);
    imshow(app_win_name, img);
    //moveWindow(win_name, 10, 10);
    key = waitKey(1);
    if(!keyHandler(key)){need_quit = true; return false;}
