...
private:
    ...
    // find delay
    float tresh_bright_min = 0;
    string plot_pic;
    int num_level_div = 0; // Коэфф деления для построения гистограммы
    int flash_duration = 0; // Длительность вспышки светодиода в наносекундах
    int fps = 0;
    int min_time_between_flash = 0;  //Мин. длительность между вспышками в миллисекундах (успокоение матрицы)
    int max_time_between_flash = 0; //Макс. длительность между вспышками в миллисекундах
    const char * cmd_led_on = "echo on > /dev/ttyACM0";
    const char * cmd_led_off = "echo off > /dev/ttyACM0";
    const char * cmd_led_blink_100_us = "echo 1 > /dev/ttyACM0"; //100 микросекунд
    const char * cmd_led_blink_200_us = "echo 2 > /dev/ttyACM0";
    const char * cmd_led_blink_300_us = "echo 3 > /dev/ttyACM0";
    const char * cmd_led_blink_400_us = "echo 4 > /dev/ttyACM0";
    const char * cmd_led_blink_500_us = "echo 5 > /dev/ttyACM0";
    const char * cmd_led_blink_600_us = "echo 6 > /dev/ttyACM0";
    const char * cmd_led_blink_700_us = "echo 7 > /dev/ttyACM0";
    const char * cmd_led_blink_800_us = "echo 8 > /dev/ttyACM0";
    const char * cmd_led_blink_900_us = "echo 9 > /dev/ttyACM0";
    time_point<system_clock> time_flash_start;
    time_point<system_clock> time_img_recive;
    int num_flash = 0; //Число вспышек светодиода, которое нужно получить
    int num_send = 0; //отправлено вспышек
    int num_receive = 0; //получено вспвшек
    int num_lost = 0; //потеряно вспвшек
    int num_wait = 0; //Кадров ожидаем вспышки
    double SV1 = 0;
    double SV2 = 0;
    int x = 0;
    int y = 0;
    int w = 0;
    int h = 0;
    float img_led_area_1 = 0;
    cv::Rect rct_work;
    Mat img_led;
    atomic<bool> led_start_enable = false;
    unique_ptr<thread> led_ptr;
    int hist_max_len = 0;  //максимальная длина гистограммы должна соответствовать макс времени ожидания вспышки
    vector<int> results ;
    vector<int> plot ;
	...
	enum Devices
	{
		...
		IMX219_CAMERA_MIPI = 5,
        IMX477_SQUARE_CAMERA_MIPI = 6,
        RTSP_H265 = 7,
        SHARED_MEMORY = 8
    }; // -- END enum Devices
 ...
 #ifdef GUI_OK
    std::string app_win_name = "Camera delay find";
 #endif // GUI_OK
    ...
    bool mouseHandler(cv::Mat& img, uchar& key, cv::Rect2f& object_rect);
	#endif // GUI_OK
