...
int loss_col = 20;
float loss_min_max = 0; 
float loss_max_max = 1000;
float loss_min[20];
int delta_i = 1;
int first = 1;
float loss_max0 = 0.000001;
float loss_max1 = 0.6;
float koef_sm = 0.95;
        ...
        */
        ...
        if (iteration >= (iter_save_last + 100) || (iteration % 100 == 0 && iteration > 1)) {
            ...
            if (ngpus != 1) sync_nets(nets, ngpus, 0);
                        ...
                        if(first==1){loss_max_max = koef_sm*avg_loss; first = 0;}
			float l_max = 0; int k_max = 0;
			for(int k=0; k<loss_col; ++k){if(loss_min[k] > l_max){l_max = loss_min[k]; k_max = k;}}
                        if(avg_loss<l_max){loss_min[k_max] = avg_loss;}

                        int ok1 = 0; 
                        if(avg_loss<loss_max1 && avg_loss>loss_max0 && avg_loss<loss_max_max){ok1=1; loss_max_max = koef_sm*avg_loss;}

                        int ok2 = 0;
                        if(avg_loss<loss_max0 && avg_loss<l_max){ok2=1; loss_min[k_max] = avg_loss;}
			
			if(ok1==1 || ok2==1)
			{
			   char buff[256];
                           sprintf(buff, "%s/%s_%d.weights", backup_directory, base, iteration);
			   //sprintf(buff, "%s/%s_%d_%f.weights", backup_directory, base, iteration, avg_loss);
		   	   FILE *f1;
			   f1 = fopen("avg_loss.txt", "a");
			   fprintf(f1, "%d", iteration);
			   char s1[] = "; ";
			   fprintf(f1, "%s", s1);
			   fprintf(f1, "%f", avg_loss);
			   char e1[] = "\n";
			   fprintf(f1, "%s", e1);
			   fclose(f1);
                           save_weights(net, buff);
