...
#define SE_NAME_SUN "Su"
#define SE_NAME_MOON "Ch"
#define SE_NAME_MERCURY "Bu"
#define SE_NAME_VENUS "Sk"
#define SE_NAME_MARS "Ma"
#define SE_NAME_JUPITER "Gu"
#define SE_NAME_SATURN "Sh"
#define SE_NAME_URANUS "Ur"
#define SE_NAME_NEPTUNE "Ne"
#define SE_NAME_PLUTO "Pl"
#define SE_NAME_MEAN_NODE "Ra"
#define SE_NAME_TRUE_NODE "Ke"
...
#define SEI_FLG_EMBHEL                      \
  8 /* TRUE, if heliocentric earth is given \
...
#endif
#define HELGRAVCONST \
  1.32712440017987e+20           /* G * M(sun), m^3/sec^2, AA 2006 K6 */
...
#define EARTH_ROT_SPEED \
  (7.2921151467e-5 * 86400) /* in rad/day, expl. suppl., p 162 */

#define LIGHTTIME_AUNIT \
  (499.0047838362 / 3600.0 / 24.0) /* 8.3167 minutes (days) */
#define PARSEC_TO_AUNIT \
  206264.8062471 /* 648000/PI, according to IAU Resolution B2, 2016 */
...
#define PNOINT2JPL                                                    \
  {                                                                   \
    J_EARTH, J_MOON, J_MERCURY, J_VENUS, J_MARS, J_JUPITER, J_SATURN, \
        J_URANUS, J_NEPTUNE, J_PLUTO, J_SUN,                          \
  }

/* planetary radii in meters */
#define NDIAM (SE_VESTA + 1)
static const double pla_diam[NDIAM] = {
    1392000000.0,   /* Sun */
    3476300.0,      /* Moon */
    2439000.0 * 2,  /* Mercury */
    6052000.0 * 2,  /* Venus */
...
static const struct aya_init ayanamsa[] = {
    /* 0: Fagan/Bradley (Default)
         "The American Sidereal Ephemeris, 1976-2000" (Astro Computing Services,
       1981) states on S.V.P. ("Synetic Vernal Point"): "The S.V.P. is the
       Sidereal longitude of the Vernal Equinox (the Tropical zero-point) in the
       Fagan-Bradley school of Western Sidereal astrology. It was determined
       empirically, its mean value being defined as 335°57'28".64 for the epoch
       1950.0." Fagan/Firebrace, "Primer of Sidereal Astrology", p. 13: "It was
       during 1957 that Garth Allen .... experimenting ... But when progressed
       for the dates of the calamities, all were found by him to be slightly
       out, the mean error being equivalent to an increase of 0°06'05" in the
       then-adopted sidereal longitude of the vernal point, determined from
       Spica in 29 Virgo (i.e. 29°06'05" Virgo; D.K.), and the proper motion
         having been allowed for. In short, for the epoch 1950.0 he proposed as
       the mean longitude of the vernal point 335°57'28.64", proper motion being
         disregarded."
         ...
         the star. */
    {2433282.42346, 24.042044444, FALSE,
     SEMOD_PREC_NEWCOMB},  // 0: Fagan/Bradley
    /*************************/
    /* 1: Standard Lahiri
    ...
    {2435553.5, 23.250182778 - 0.004658035, FALSE,
     SEMOD_PREC_IAU_1976},  // 1: Lahiri
    /*************************/
    ...
    /* 3: B.V. Raman (Robert Hand)
         ...
         pp. 279, 287.
         This ayanamsha is apparently not based on a valid precession theory
       (e.g.
    ...
    /* 6: Djwhal Khool (Graham Dawson),
         ...
         See Philipp Lindsay, “The Beginning of the Age of Aquarius: 2,117
       A.D.”,
    ...
    /* 7: Shri Yukteshwar; (David Cochrane)
         ...
         the revised edition of 1977) At the same time he believed that this was
       the distance of the spring equinox from the star Revati, which he put at
       the initial point of Aries.  Unfortunately, this is wrong, because on
       that date Revati was actually 18°23' away from the vernal point. The
       error is explained from the fact that Yukteshwar used the zero ayanamsha
       year 499 CE and an inaccurate Suryasiddhantic precession rate of
       360°/24'000 years = 54 arcsec/year. It is obvious that Yukteshwar
       actually intended an ayanamsha that starts at the star Revati.  */
    {J1900, 360 - 338.917778, FALSE, -1},  // 7: Shri Yukteshwar
    //{2412543.5, 20.91, TRUE, -1},        // 7: Shri Yukteshwar; (Holy Science,
    // p. xx)
    /*************************/
    ...
    /* 12: Babylonian, Huber
       ...
       Centaurus 1958, 5, p. 192-208. This ayanamsha had a wrong initial value
       until 14 Sept. 2018. */
    ...
    /*************************/
    /* 21: Suryasiddhanta, assuming ingress of mean Sun into Aries at point of
       mean equinox of date on 21.3.499, near noon, Ujjain (75.7684565 E) =
       7:30:31.57 UT = 12:33:36 LMT*/
    {1903396.8128654, 0, TRUE, 0},  // 21:
    /*************************/
    /* 22: Suryasiddhanta, assuming ingress of mean Sun into Aries at true
    ...
    /*************************/
    /* 30: R. Gil Brand; Galactic Center at golden section between 0 Sco and 0
       Aqu; note: 0° Aqu/Leo is the symmetric axis of rulerships */
    ...
    /*************************/
    /* 32: Galactic Equator True, i.e. galactic/ecliptic intersection point
       based on the galactic pole as given in: Liu/Zhu/Zhang, „Reconsidering the
         galactic coordinate system“, A & A No. AA2010, Oct. 2010 */
    {0, 0, FALSE, 0},  // 32: GE true
    /*************************/
    /* 33: Galactic Equator Mula, i.e. galactic/ecliptic intersection point in
       the middle of lunar mansion Mula */
    ...
    /*************************/
    /* 42: Vettius Valens (Moon; derived from Holden 1995 p. 12 for epoch of
       Valens 1 Jan. 150 CE julian) */
    ...
    /*************************/
    /* 44: Lahiri (VP285), mean sun at 360° in 285CE; epoch for mean sun at 0
       acc. to Simon 1994, corrected for Vondrak precession
    ...
    //{1827424.752255678, 0.0, FALSE, SEMOD_PREC_NEWCOMB}, // 45: Krishnamurti
    // VP291
    ...
    /*************************/
    /*{2061539.789532065, 6.83333333, TRUE, -1}, *41: Manjula's Laghumanasa, 10
       March 932, 12 PM LMT Ujjain (75.7684565 E), ayanamsha = 6°50' */
    /* */
    {J1900, 0, FALSE, -1}, /*46: - */
};
