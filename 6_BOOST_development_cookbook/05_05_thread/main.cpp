...
void fill_file_with_data(char fill_char, std::size_t size,
                         const char* filename) {
...
void example_with_threads() {
  ...
  // Somewhere in thread that draws a user interface
  if (is_first_run()) {
    boost::thread(boost::bind(&fill_file_with_data, 'S', 8 * 1024 * 1024,
                              "save_file.txt"))
        .detach();
  }
}
...
void example_with_joining_threads() {
  ...
  // Somewhere in thread that draws a user interface
  if (is_first_run()) {
    boost::thread t(boost::bind(&fill_file_with_data, 'B', 8 * 1024 * 1024,
                                "save_file.txt"));
...
void set_not_first_run() { g_is_first_run = false; }

bool is_first_run() { return g_is_first_run; }
