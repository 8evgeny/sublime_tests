...
Метод run() неблокирующий, запускает работу внутреннего потока. Требования к
исключениям аналогичны классу потока.

Метод stop() сигнализирует о необходимости завершить работу внутреннего потока.
Неблокирующий, потокобезопасный.

Метод wait_shutdown() ожидает завершения работы внутреннего потока (аналог
метода join() класса потока).
