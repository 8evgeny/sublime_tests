...
class CThread {
 ...
 public:
  ...
  virtual void set_iteration_callback(
...
/*
Обратить внимание на стиль оформления класса. В дальнейшем придерживаться
данного стиля. Стиль скобок функций и методов классов:

    void function()
{
}
Обратить внимание на использование пробелов с операторами. Имена методов и
переменных должны корректно отражать их назначение, для облегчения чтения кода и
поиска ошибок.

Использование идиомы pimpl не требуется.
Деструктор должен корректно останавливать работающий поток, не генерировать
исключения.

Метод run() - неблокирующий, запускает внутренний поток. При
невозможности корректно запустить поток, генерирует исключение. При нормальном
запуске потока возвращает управление вызвавшему коду.

Метод stop() - неблокирующий, потокобезопасный. Сигнализирует объекту о
необходимости завершить работу внутреннего потока, без фактического ожидания
остановки.
...
Метод set_iteration_callback(const std::function<void()> &
on_iteration_callback) задает callback — функциию, которую объект будет вызывать
